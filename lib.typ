...
// Custom bold(upright( argument )) function in math mode to make x
#let bup(body) = math.bold(math.upright(body))
...
#let university-assignment(
  ...
  set page(
    numbering: (current, total) => [#current / #total],
    number-align: right + bottom,
  )


  // links → slightly blue + underlined
  show link: it => [
    #text(fill: rgb(0, 90, 180))[
      #underline(it.body)
    ]
  ]
  ...
  // references → slightly blue (no underline)
  show ref: it => [
    #text(fill: rgb(0, 90, 180))[
      #it
    ]
  ...
  show raw.where(block: false): it => box(
    ...
    outset: (y: 3pt),
    radius: 2pt,
  )[#text(font: "Cascadia Mono", size: 0.9em)[#it]]
