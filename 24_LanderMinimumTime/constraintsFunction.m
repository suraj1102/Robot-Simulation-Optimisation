function [c, ceq] = constraintsFunction(zfmin, problem)
    [g, zinit, zfinal, numPoints] = unpack_p(problem);
    c = [];
    ceq = [];

    N = numPoints;

    [x, z, vx, vz, ux, uz, T] = unpack_zfmin(zfmin, problem);

    dt = T / N;

    % Boundary Conditions
    ceq = [ceq; x(1) - zinit(1)];
    ceq = [ceq; z(1) - zinit(2)];
    ceq = [ceq; vx(1) - zinit(3)];
    ceq = [ceq; vz(1) - zinit(4)];

    ceq = [ceq; x(end) - zfinal(1)];
    ceq = [ceq; z(end) - zfinal(2)];
    ceq = [ceq; vx(end) - zfinal(3)];
    ceq = [ceq; vz(end) - zfinal(4)];

    % Dynamics - Trapezoidal Discretisation
    for i = 1:N
        ceq = [ceq; x(i+1) - ( x(i) + dt/2 * (vx(i) + vx(i+1)) )];
        ceq = [ceq; z(i+1) - ( z(i) + dt/2 * (vz(i) + vz(i+1)) )];
        ceq = [ceq; vx(i+1) - ( vx(i) + dt/2 * (ux(i) + ux(i+1)) )];
        ceq = [ceq; vz(i+1) - ( vz(i) + dt/2 *( (uz(i) - g) + (uz(i+1) - g) ) )];
    end

    % Do not hit the ground - c(x) <= 0 i.e -z(i) <= 0
    for i = 1:N+1
        c = [c; -z(i)];
    end

    % Control constraint - norm(control) <= umax
    umax = problem.umax;
    for i = 1:N+1
        c = [c; norm([ux(i); uz(i)]) - umax ];
    end
end